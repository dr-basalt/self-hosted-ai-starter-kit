...
# Script de configuration Let's Encrypt pour OpenWebUI
# Usage: ./setup-letsencrypt.sh [domain] [email]

set -e

# Variables par défaut
DOMAIN=${1:-"taz.infra.ori3com.cloud"}
EMAIL=${2:-"admin@taz.infra.ori3com.cloud"}

echo "🔐 Configuration Let's Encrypt pour OpenWebUI"
echo "=============================================="
echo "Domaine: $DOMAIN"
echo "Email: $EMAIL"
echo ""
...
certbot certonly \
    --standalone \
    --pre-hook "docker compose stop openwebui" \
    --post-hook "docker compose start openwebui" \
    --email "$EMAIL" \
    --agree-tos \
    --no-eff-email \
    --domains "$DOMAIN"
...
0 12 * * * root certbot renew --quiet --pre-hook "docker compose stop openwebui" --post-hook "docker compose start openwebui && cp /etc/letsencrypt/live/$DOMAIN/fullchain.pem /app/certs/cert.pem && cp /etc/letsencrypt/live/$DOMAIN/privkey.pem /app/certs/key.pem && chmod 644 /app/certs/cert.pem && chmod 600 /app/certs/key.pem"
EOF
